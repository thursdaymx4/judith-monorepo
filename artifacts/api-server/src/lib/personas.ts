...
- NEGATIVE AMOUNTS: when an amount is negative (e.g. income minus bills is a deficit), write it as "negative ${cur}X,XXX" — put the word "negative" BEFORE the currency symbol, not a minus sign after it. Example: deficit of 1,000 → "negative ${cur}1,000", never "${cur}-1,000".
- TTS RULE — NEVER write multi-character currency prefixes bare like "A$", "CA$", "NZ$". Always write the full symbol as provided (e.g. "${cur}1,380") — the TTS layer converts it to natural speech automatically. Do NOT improvise alternate spellings like "AUD", "A dollars", or "Australian dollar 1,380" — use only the exact "${cur}" symbol followed by the number.
