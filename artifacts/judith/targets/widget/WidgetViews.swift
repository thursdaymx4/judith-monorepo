        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            if let bill = entry.nextBill {
                ...
                Text(bill.amountDisplay(currency: entry.currency))
                    .font(.title2.weight(.black).design(.monospaced))
            ...
            if entry.unpaidCount > 0 {
                VStack(spacing: 7) {
                    ...
                    HStack {
                        ...
                        Spacer()
                        Text("\(entry.currency)\(String(format: "%.0f", entry.totalOwed))")
                            .font(.title3.weight(.black).design(.monospaced))
        ...
        ZStack {
            if let bill = entry.nextBill {
                ...
                let gaugeValue = Text(bill.dueDays <= 0 ? "!" : "\(bill.dueDays)")
                    .font(.body.weight(.bold).design(.monospaced))
                    .foregroundStyle(bill.urgency.color)
        ...
        HStack(alignment: .center, spacing: 9) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                if let bill = entry.nextBill {
                    ...
                    Text(bill.provider)
                        .font(.footnote.weight(.bold).design(.rounded))
                        .foregroundStyle(.primary)
                        .lineLimit(1)

                    HStack(spacing: 3) {
                        Text(bill.amountDisplay(currency: entry.currency))
                            .font(.callout.weight(.black).design(.monospaced))
                            .foregroundStyle(bill.urgency.color)
                        Spacer()
                        Text("\(entry.unpaidCount) due")
                            .font(.system(size: 8, design: .rounded))
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text("All bills paid!")
                    .font(.callout.weight(.bold).design(.rounded))
                    .foregroundStyle(Color.judithAccent)
