...
export const JUDITH_VOICE = {
  ...
  billFlow: {
    ...
    marites: {
      breather0:  { en: "Oh my gosh! Essentials all saved — power, water, internet, done!", fil: "Grabe! Essentials naka-save na! Kuryente, tubig, internet — done na. Hinga muna." },
      breather1:  { en: "Subscriptions logged! All those sneaky little charges — we see you now!", fil: "Mga subscriptions, naka-save! Wala nang makakalimutan." },
      breather2:  { en: "Insurance too?! Okay, you are so on top of it — I love this!", fil: "Ay! Insurance, naka-log na rin! Grabe ka besh, ang haba ng listahan mo!" },
      countCards: { en: "Oh my gosh, credit cards! Okay, how many do you have?!", fil: "Ngayon ang mga credit card. Ilan ang mayroon ka? Isa-isahin natin." },
      countLoans: { en: "Loans too?! Okay, how many are we dealing with?", fil: "At loans — personal, car, bahay, kahit ano. Ilan?" },
      more:       { en: (n) => `${n} bills and counting! Any more we need to add?`, fil: (n) => `${n} na bill na logged. May iba pa? Cards, loans, gym, insurance?` },
      cardFirst:  { en: "Okay, first card — which bank, how much is due, and the date?", fil: "Sa una mong card, what's your bank, due date and amount due?" },
      cardNext:   { en: (n) => `Card ${n} — same deal, which bank and amount?`, fil: (n) => `Card ${n} — same details: bank, amount, at kelan due?` },
      loanFirst:  { en: "First loan — who's the lender, monthly amount, and due date?", fil: "Sinong nag pahiram sayo, magkano monthly at tuwing kelan mo kailangan bayaran?" },
      loanNext:   { en: (n) => `Loan ${n} — lender and monthly amount?`, fil: (n) => `Loan ${n} — sino pa, magkano monthly, at kelan?` },
    },
    britney: {
